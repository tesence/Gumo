...
class RandomizerLeague(commands.Cog, name="Randomizer League"):
    ...
    async def _get_submissions(self, date: datetime):
        ...
        return [r['Runner'] for r in records if r['Date'] == date]
    ...
    async def league_set(self, interaction: discord.Interaction,
        ...
        """
        week_start_date = get_week_start_date(week_start_date) or get_current_week_start_date()
        async with asqlite.connect(DB_FILE) as connection:
            ...
            query = "INSERT INTO league_settings VALUES (?, ?, ?) " \
                    "ON CONFLICT(week_start_date, name) DO UPDATE SET value = excluded.value " \
                    "ON CONFLICT(week_start_date, value) DO NOTHING;"
    ...
    async def league_view(self, interaction: discord.Interaction,
        ...
        week_start_date = get_week_start_date(week_start_date) or get_current_week_start_date()
        async with asqlite.connect(DB_FILE) as connection:
            query = "SELECT * FROM league_settings WHERE week_start_date = ?;"
    ...
    async def league_clear(self, interaction: discord.Interaction,
        ...
        week_start_date = get_week_start_date(week_start_date) or get_current_week_start_date()
        async with asqlite.connect(DB_FILE) as connection:
            query = "DELETE FROM league_settings WHERE week_start_date = ?;"
